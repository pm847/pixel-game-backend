...
var json = require('./wrongwin.json');
...
function computeResult(raw_json) {
  ...
  for (let i = 0; i < remain.length; i++) {
    ...
    if (remain[i] >= 2) {
      ...
      var tmp_player = players[i];
      tmp_player.x = raw_json.origin.x;
      tmp_player.y = raw_json.origin.y;
      replayers.push(tmp_player);
    }
  }
  ...
  for (let i = 0; i < raw_json.goals.length; i++) {
    for (let j = 0; j < remain.length; j++) {
      if (raw_json.goals[i].x == replayers.x && raw_json.goals[i].y == replayers.y) {
        raw_json.goals[i].is_achieved = true;
      }
    }
  }
  if (!replayers.length) {
    ...
    var finished = true;
    for (let i = 0; i < raw_json.goals.length; i++) {
      if (!raw_json.goals[i].is_achieved) {
        finished = false;
      }
    }
